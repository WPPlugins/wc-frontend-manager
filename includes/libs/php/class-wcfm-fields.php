...
class WCFM_Fields {
  ...
  public function upload_input($field) {
    ...
    if($field['mime'] == 'image') {
    	$img_src = $field['value'];
    	//if( !$img_src ) $img_src = $WCFM->plugin_url . 'includes/libs/upload/images/Placeholder.png';
      $mimeProp = '<img id="'.esc_attr($field['id']).'_display" src="'.esc_attr( $img_src ).'" width="'.absint( $field['prwidth'] ).'" class="'.$placeHolder.'" />';
    } else {
      if($field['value'])
  ...
  public function multi_input($field) {
    ...
    printf(
      '<div id="%s" class="%s multi_input_holder" data-name="%s" data-length="%s" %s>',
      $field['id'],
      $field['class'],
      $field['name'],
      count($field['value']),
